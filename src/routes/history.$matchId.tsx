    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-2xl px-4 py-10 text-center sm:px-6">
          ...
          <Link to="/history" className="mt-4 inline-block text-primary hover:underline">
            Back to history
          </Link>
        </div>
      </AppShell>
        ...
        <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {format(new Date(match.date), "MMMM d, yyyy")} · {match.rounds} rounds ·{" "}
            ...
            <Card key={t.id} className={`p-5 ${match.winner === t.id ? "border-success" : ""}`}>
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{ background: t.id === "A" ? "var(--team-a)" : "var(--team-b)" }}
                />
              ...
              <div className="mt-3 space-y-1.5">
                {t.players.map((p) => (
                  <PlayerBadge key={p} name={p} size="sm" status="confirmed" />
                ))}
              </div>
            </Card>
          ))}
        ...
        <section className="mt-8">
          ...
          <Card className="mt-3 divide-y divide-border">
            <div className="flex items-center justify-between p-3">
              <PlayerBadge name="Adam" status="confirmed" />
              <Badge variant="secondary">Registered</Badge>
            </div>
            <div className="flex items-center justify-between p-3">
              <PlayerBadge name="Kuba" status="confirmed" />
              <Badge variant="secondary">Registered</Badge>
            </div>
            <div className="flex items-center justify-between p-3">
              <PlayerBadge name="Bary" isGuest />
              <Badge variant="outline">Guest</Badge>
            </div>
            <div className="flex items-center justify-between p-3">
              <PlayerBadge name="Michał" status="pending" />
              <Badge variant="outline">Manually added</Badge>
            </div>
          </Card>
        </section>
