...
function Rankings() {
  ...
  const sortedPlayers = [...playerRanking].sort((a, b) => {
  ...
  const sortedPairs = [...pairStats].sort((a, b) => b.winRate - a.winRate).map((p, i) => ({ ...p, rank: i + 1 }));

  return (
    <AppShell>
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
        <h1 className="font-display text-3xl font-bold">Rankings</h1>
        <p className="mt-1 text-sm text-muted-foreground">No minimum games required — every player and pair counts.</p>
          ...
          <TabsContent value="players" className="mt-4">
            <Card className="overflow-hidden">
              <div className="grid grid-cols-[40px_1fr_60px_60px_60px_70px_80px] items-center gap-2 border-b border-border bg-muted/40 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                <span>#</span><span>Player</span><span className="text-right">G</span><span className="text-right">W</span><span className="text-right">L</span><span className="text-right">Win %</span><span className="text-right">Avg</span>
              </div>
              {sortedPlayers.map((p) => (
                <div key={p.username} className="grid grid-cols-[40px_1fr_60px_60px_60px_70px_80px] items-center gap-2 border-b border-border px-4 py-3 text-sm last:border-0 tabular-nums">
                  <span className="flex items-center gap-1 font-bold">{p.rank} {rankIcon(p.rank)}</span>
          ...
          <TabsContent value="pairs" className="mt-4">
            <Card className="overflow-hidden">
              <div className="grid grid-cols-[40px_1fr_60px_60px_60px_70px] items-center gap-2 border-b border-border bg-muted/40 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                <span>#</span><span>Pair</span><span className="text-right">G</span><span className="text-right">W</span><span className="text-right">L</span><span className="text-right">Win %</span>
              </div>
              {sortedPairs.map((p) => (
                <div key={p.id} className="grid grid-cols-[40px_1fr_60px_60px_60px_70px] items-center gap-2 border-b border-border px-4 py-3 text-sm last:border-0 tabular-nums">
                  <span className="flex items-center gap-1 font-bold">{p.rank} {rankIcon(p.rank)}</span>
                  <span className="font-medium">{p.player1} <span className="text-muted-foreground">+</span> {p.player2} {p.games === 1 && <Badge variant="outline" className="ml-1 text-[10px]">New</Badge>}</span>
                  <span className="text-right">{p.games}</span>
                  <span className="text-right font-semibold text-success">{p.wins}</span>
                  <span className="text-right text-muted-foreground">{p.losses}</span>
                  <span className="text-right font-bold">{Math.round(p.winRate * 100)}%</span>
